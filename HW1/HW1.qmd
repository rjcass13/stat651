...
$P(C | p) = \frac{P(p | C)P(C)}{P(p)} = \frac{.98(.001)}{.00178} = .551$

### b) 

Given the likelihood of having cancer given a positive result is only around 1/2, I would suggest to the researchers that they either repeat the test multiple times on a subject (useful if test is cheap/fast) or find a way to reduce the chance of a positive result when the patient doesn't have cancer (given the current values, that will have a larger impact than increasing positive results if they do have cancer).
...
calc_utility <- function(N, theta) {
  if (N <= 100) {
    ...
    bad_N <- 101:N
    prob_bad_N <- 1- mapply(pbinom, q = bad_N, size = N, prob = theta)
...
Given a $\theta$ of .95, the optimal value of $N$ to maximize money made is 104 tickets.
